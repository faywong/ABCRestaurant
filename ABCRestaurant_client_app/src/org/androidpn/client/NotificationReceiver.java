    ...
    public void onReceive(Context context, Intent intent) {
        ...
        if (Constants.ACTION_SHOW_NOTIFICATION.equals(action)) {
            ...
            if (notificationTitle.equals(ABCConfig.PUSH_COURSE_TO_COOKER_TITLE)) {
				...
				if (ids.length == 2) {
	            	cookIntent.putExtra("course_id", Integer.parseInt(ids[0]));
	            	cookIntent.putExtra("order_id", Integer.parseInt(ids[1]));
	                Log.d(LOGTAG, "course_id = " + ids[0]);
	                Log.d(LOGTAG, "order_id = " + ids[1]);
				}
				else {
		            Log.d(LOGTAG, "ids = " + ids);
				}
            	cookIntent.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
            	notificationMessage = msgPrefix + notificationMessage;
				try {
            	...
            	//context.sendBroadcast(cookIntent);
				return;
            }
